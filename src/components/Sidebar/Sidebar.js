	...
	return (
		<aside className={className}>
			...
			<Badge
				...
				image_url={user ? require(`../../img/pp/${user?.id}.${user?.image_extension}`) : require("./pp_redaction.png")}
			/>
		</aside>
	);
})`
...
const UpdateNotification = styled(({ className, date }) => {
	function formatDate(dateString) {
		if (dateString) {
			...
			const formattedDate = formatter.format(date).replace(/\b\w/g, (l) => l.toUpperCase());
